...
import Item from "../components/Elements/Item/Item";
...
const Page: React.FC = () => {
  ...
  const fetchItem = async () => {
    const response = await axios.get(
      `http://localhost:3000/itemSets?id=${setId}`
    );
    setItemsSet(response.data.data);
    const itemsResponse = await axios.get(
      `http://localhost:3000/items?setId=${setId}`
    );
    ...
    <div>
      ...
      <div className="bg-[#a7a18d] h-full pb-10">
        <div className=" text-black pt-10">
          <div className="px-8">
            {(itemsSet as mysql.RowDataPacket).map(
              (itemSet: GroupedNmbItems) => (
                <div className="mb-8">
                  <div className="gap-8 justify-center grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                    <div className="bg-[#cfc4ab] w-full border-[#3eb167] border py-2 px-3">
                      ...
                      {(items as mysql.RowDataPacket).map(
                        (item: GroupedItems) => (
                          <div
                            className="mt-3 flex items-center cursor-pointer hover:font-bold hover:bg-[#779643] rounded-lg transition-all p-1"
